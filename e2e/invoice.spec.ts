...
test.describe("the invoice fits its page", () => {
  test("a domestic invoice with SAC codes and the declarations", async ({
    page,
  }) => {
    ...
    // The discount, and the taxable value it produced. CGST s.15(3)(a) only
    // deducts a discount recorded in the invoice, so a row that fails to print
    // is a discount that legally did not happen.
    await expect(sheet).toContainText("discount (10%)");
  ...
  test("an export invoice with the wire-transfer block", async ({ page }) => {
    ...
    await expect(sheet).toContainText("IGST not charged");
    await expect(sheet).toContainText(
      "account for tax in their own jurisdiction",
    );
  ...
  test("an export invoice prints on one sheet", async ({ page }) => {
    ...
    const pages = (
      pdf.toString("latin1").match(/\/Type\s*\/Page(?![s/\w])/g) ?? []
    ).length;
    expect(pages).toBe(1);
  });
});
