 ...
 */
const renderContract = (doc: ContractDocument) => render(<>{contractBlocks(doc)}</>);
...
describe('contractBlocks', () => {
  ...
  it('prints a filled blank as ordinary text', () => {
    renderContract(contractDoc({ codes: ['05'], blanks: { 'part.05.limits#1': '120' } }));
  ...
  it('signs the Agreement before the first Schedule, not after the last', () => {
    ...
    const schedule = screen.getByLabelText('Schedule A');
    expect(
      execution.compareDocumentPosition(schedule) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
  });
  ...
  it('breaks a long clause between its points', () => {
    renderContract(contractDoc());
    const section = screen.getByText('3. Mutual Obligations').closest('section')!;
   ...
   */
  it('prints a clause number as its own semibold marker', () => {
    renderContract(contractDoc());
  ...
  describe('pages of its own', () => {
    ...
    it('opens each Schedule on a page of its own', () => {
      renderContract(contractDoc({ codes: ['01', '11'] }));
      expect(screen.getByLabelText('Schedule A')).toHaveAttribute('data-page', 'own');
      expect(screen.getByLabelText('Schedule B')).toHaveAttribute('data-page', 'own');
    });
    ...
    it('keeps the Schedule covers white', () => {
      ...
      expect(screen.getByLabelText('Schedule A')).not.toHaveAttribute('data-page-frame');
    });

    /** Two parties side by side need the sheet's full width, not a column. */
    it('gives every signature block a page of its own', () => {
      renderContract(contractDoc({ codes: ['01'] }));
      expect(screen.getByLabelText('Execution')).toHaveAttribute('data-page', 'own');
